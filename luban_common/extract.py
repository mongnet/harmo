...
import jmespath
import re
from luban_common import exceptions
...
def extract_by_object(response: Response, extract_expression: str):
    ...
    elif extract_expression.startswith('resp'):
        try:
            response_parse_dict = response.json()
            return extract_by_jmespath({"resp": response_parse_dict}, extract_expression)
...
def extract_by_jmespath(extract_obj: dict, extract_expression: str):  # noqa
    ...
    :param extract_expression: eg: 'resp.code'
